...
In `prepare_domain()`, the first parameter is the domain data frame followed by the two letter domain code.
...
`create_participant_table()` creates a one row per participant analysis table along with several participant characteristic variables and baseline test information. World Health Organisation child growth standards are also derived if age, sex, height and weight are in the dataset; height for age (`HAZ`), weight for age (`WAZ`) and weight for height (`WHZ`) $z$-scores are calculated for participants under the age of 5, with respective flags (i.e. `HAZ_FLAG`) for scores outside a typical range.
...
-   Paper: ['iddoverse: An R package for converting IDDO-SDTM data into analysis datasets'](https://github.com/Infectious-Diseases-Data-Observatory/iddoverse/tree/main/paper)
