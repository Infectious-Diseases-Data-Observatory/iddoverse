...
![Flowchart of functions within the `iddoverse` package. SDTM domains are indiviudally transformed using `prepare_domain`. Then, the output could either be used as it is, or combined with other transformed domains into analysis datasets using `create_participant_table` or `create_clinical_table`.](figures/flowchart.tif.png)
